...
public final class SnappyRawCompressor
{
            ...
            final short[] table)
    {
        ...
        long output = writeUncompressedLength(null, outputAddress, (int) (inputLimit - inputAddress));
        ...
        for (long blockAddress = inputAddress; blockAddress < inputLimit; blockAddress += BLOCK_SIZE) {
            ...
            while (input <= fastInputLimit) {
                ...
                for (input += 1; input + (skip >>> 5) <= fastInputLimit; input += ((skip++) >>> 5)) {
                    ...
                    int currentInt = littleEndian(UNSAFE.getInt(null, input));
                    ...
                    if (currentInt == littleEndian(UNSAFE.getInt(null, candidateIndex))) {
                        break;
                    }
                }
                ...
                output = emitLiteralLength(null, output, literalLength);
                ...
                output = fastCopy(null, nextEmitAddress, null, output, literalLength);
                ...
                do {
                    ...
                    int matched = count(null, input + SIZE_OF_INT, candidateIndex + SIZE_OF_INT, blockLimit);
                    matched += SIZE_OF_INT;

                    // Emit the copy operation for this chunk
                    output = emitCopy(null, output, input, candidateIndex, matched);
                    input += matched;
                    ...
                    long longValue = littleEndian((UNSAFE.getLong(null, input - 1)));
                ...
                } while (inputBytes == littleEndian(UNSAFE.getInt(null, candidateIndex)));
...
//                UNSAFE.copyMemory(inputBase, nextEmitAddress, outputBase, output, literalLength);
                UNSAFE.copyMemory(nextEmitAddress, output, literalLength);
